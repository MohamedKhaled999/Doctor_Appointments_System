...
namespace Services.MappingProfiles
{
    internal class AppointmentMappingProfile : Profile
    {
        ...
        public AppointmentMappingProfile(IUnitOfWork unitOfWork)
        {
            CreateMap<Appointment, AppointmentDTO>()
                ...
                .AfterMap((src, dest) =>
                {
                    dest.IsExists = unitOfWork.GetRepository<Review, int>()
                                    .GetAllAsync(new AppointmentReviewSpecifications(r => r.DoctorID == src.DoctorReservation.DoctorID && r.PatientID == src.PatientId)).Result.Any();
                });
        }
    }
}
